...
class ExplorationPlanner(AbstractPlanner):
    ...
    def plan_next_action(
        ...
        # Filter candidates: visible, enabled, not previously visited, not repeatedly
        # failed, and never a CONTENT node (those exist for claim verification only -
        # clicking a heading would burn an action for nothing).
        candidates = [
            e for e in available_elements
            if e.is_visible
            and e.is_enabled
            and e.element_type != ElementType.CONTENT
