...
class ExplorerConfig(BaseSettings):
    ...
    # 10s was too tight in practice: a cold TLS handshake to a slow origin from a
    # cloud container regularly exceeded it and failed the whole crawl.
    navigation_timeout_ms: int = Field(default=30000, description="Page navigation timeout in ms")
    action_timeout_ms: int = Field(default=8000, description="Action execution timeout in ms")
