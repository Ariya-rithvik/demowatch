...
class DOMAnalyzer(AbstractDOMAnalyzer):
    ...
    async def extract_elements(self) -> List[UIElement]:
        ...
        () => {
            ...
            // Text that carries checkable claims (prices, headings, feature bullets).
            // Leaf-only so nested containers don't duplicate their children's text.
            const contentQuery = 'h1, h2, h3, h4, h5, h6, li, dd, dt, th, td, figcaption, p, span, strong, b, em';
            const MAX_CONTENT_NODES = 200;

            const nodes = [];
            document.querySelectorAll(interactiveQuery).forEach(el => nodes.push({el: el, isContent: false}));

            // Rank before capping: taking the first N in document order would drop
            // pricing, which usually sits far down the page.
            const priceRe = /[$£€¥₹]\s?\d|\d+(?:[.,]\d+)?\s?(?:USD|EUR|GBP|INR)\b/i;
            const contentCandidates = [];
            for (const el of document.querySelectorAll(contentQuery)) {
                if (el.children.length > 0) continue;          // leaf nodes only
                const t = (el.innerText || '').trim();
                if (!t || t.length > 180) continue;
                const tag = el.tagName.toLowerCase();
                const rank = priceRe.test(t) ? 0
                           : /^h[1-6]$/.test(tag) ? 1
                           : tag === 'li' ? 2
                           : /\d/.test(t) ? 3
                           : 4;
                contentCandidates.push({el: el, rank: rank});
            }
            contentCandidates.sort((a, b) => a.rank - b.rank);
            contentCandidates.slice(0, MAX_CONTENT_NODES)
                             .forEach(c => nodes.push({el: c.el, isContent: true}));
            ...
            nodes.forEach(({el, isContent}, index) => {
        ...
        for raw in raw_elements:
            ...
            # Content nodes are captured for claim verification only; typing them as
            # CONTENT keeps the planner from ever treating them as click targets.
            elem_type = (
                ElementType.CONTENT if raw.get("isContent")
                else self._classify_element(tag, raw["attributes"])
            )
